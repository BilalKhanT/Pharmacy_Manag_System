...
class Window1:
    ...
    def __init__(self, master):
        ...
        self.lblTitle = Label(self.frame, font=("arial", 50, "bold",), text="COMSATS KI PHARMACY", bd=20)
    ...
    def LoginSystem(self):
        ...
        if (user == str("SAJIDA")) or (user == str("sajida")) or (user == str("Sajida")) and (password == str(111)):
            self.btnReg.config(state=NORMAL)
            self.btnPhar.config(state=NORMAL)

        elif (user == str("BILAL")) or (user == str("bilal")) or (user == str("Bilal")) and (password == str(222)):
            self.btnReg.config(state=NORMAL)
            self.btnPhar.config(state=NORMAL)


        elif (user == str("ABDUL WAHEED")) or (user == str("abdul waheed")) or (user == str("Abdul Waheed")) and (
                password == str(333)):
            self.btnReg.config(state=NORMAL)
            self.btnPhar.config(state=NORMAL)

        else:
            tkinter.messagebox.askyesno("COMSATS Ki Pharmacy", "Invalid username or password, please try again.")
    ...
    def iExit(self):
        self.iExit = tkinter.messagebox.askyesno("COMSATS Ki Pharmacy", "Please confirm to exit!")
        if self.iExit > 0:
            self.master.destroy()
        return
